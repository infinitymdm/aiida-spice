...
def test_run(spice_code):
    ...
    if not spice_code:
        ...
        spice_code = helpers.get_code(
            entry_point="spice", computer=computer
        )
    ...
    # future = submit(CalculationFactory('spice'), **inputs)
    result = engine.run(CalculationFactory("spice"), **inputs)
